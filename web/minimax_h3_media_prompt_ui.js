...
const TEXT = {
    ...
    previewTitle: ZH_BROWSER ? "\u6700\u7ec8\u63d0\u793a\u8bcd" : "Final prompt",
...
function localizeNodeInstance(node) {
    ...
    if (isOutput(node)) {
        ...
        for (const input of node.inputs || []) {
            if (input.name === "media_prompt") setLocalizedSlotLabel(input, ZH_BROWSER ? "\u5a92\u4f53\u63d0\u793a\u8bcd" : "Media prompt");
        }
        return;
    }
    if (!isTarget(node)) return;
    node.title = TEXT.mainTitle;
    const labels = { text: TEXT.prompt };
...
function getWidget(node, name) {
    const resolvedName = name === "prompt" ? "text" : name;
    return node?.widgets?.find((widget) => widget?.name === resolvedName) || null;
}
...
function resequence(node) {
    const counts = { image: 0, video: 0, video_audio: 0, audio: 0 };
...
function normalizeLinks(node, removeMissing = true) {
    ...
    for (const link of links) {
        ...
        if (source) {
            const inferredType = getMediaType(getSlotType(source.outputs?.[sourceSlot]), source);
            if (inferredType === "video" || inferredType === "video_audio") mediaType = inferredType;
        }
        if (!["image", "video", "video_audio", "audio"].includes(mediaType)) continue;
    ...
    for (let index = normalized.length - 1; index >= 0; index -= 1) {
        ...
        if (link.media_type !== "video_audio" || videoSourceIds.has(link.source_id)) continue;
        seen.delete(`${link.source_id}:${link.source_slot}:video_audio`);
    ...
    for (const link of [...normalized]) {
        ...
        const key = `${link.source_id}:${audioSlot}:video_audio`;
        if (seen.has(key)) continue;
        seen.add(key);
        normalized.push({
            source_id: link.source_id,
            source_slot: audioSlot,
            source_type: getSlotType(source.outputs[audioSlot]) || "AUDIO",
...
function getMediaType(sourceType, sourceNode = null) {
    ...
    if (name === "vhs_loadvideo" || name === "vhs_loadvideoffmpeg") {
        if (type.includes("AUDIO")) return "video_audio";
        if (type.includes("IMAGE")) return "video";
    }
...
function mediaLimits(node) {
    return { image: 9, video: 3, video_audio: 3, audio: 3, total: MAX_MEDIA };
}
...
function createResourceNode(canvas, targetNode, mediaType, position) {
    ...
    if (mediaType === "video") {
        ...
        if (audioSlot >= 0) {
            const audioOutput = node.outputs[audioSlot] || {};
            addVirtualLink(targetNode, node, audioSlot, getSlotType(audioOutput) || "AUDIO", "video_audio");
        }
    }
    graph.setDirtyCanvas?.(true, true);
    return true;
...
function buildRuntimePrompt(node, runtimeLinks) {
    ...
    return doc.parts.map((part) => {
        ...
        if (index >= 0) {
            let ordinal = runtimeLinks.slice(0, index + 1).filter((link) =>
                String(link.media_type || "image").toLowerCase() === mediaType
            ).length;
            if (mediaType === "audio") {
                ordinal += runtimeLinks.filter((link) =>
                    String(link.media_type || "image").toLowerCase() === "video_audio"
                ).length;
            }
            const tag = { image: "Picture", video: "Video", audio: "Audio" }[mediaType] || "Picture";
            return `<${tag} ${ordinal}>`;
        }
...
function patchGraphToPrompt() {
    ...
    app.graphToPrompt = async function graphToPromptWithOrderedMedia() {
        ...
        for (const node of app.graph?._nodes || []) {
            ...
            const runtimeLinks = [...images, ...videos, ...videoAudios, ...audios];
            ...
            promptNode.inputs.text = buildRuntimePrompt(node, runtimeLinks);
        }
        return promptData;
    };
}
...
function mentionOptions(node) {
    ...
    const links = normalizeLinks(node).filter((link) => String(link.media_type || "image") !== "video_audio");
...
function syncEditorMode(node) {
    ...
    editor.dataset.placeholder = TEXT.referencePromptPlaceholder;
...
function setOutputPreview(node, text) {
    if (!node) return;
    const value = String(text || "");
    node.properties ||= {};
    node.properties[OUTPUT_PREVIEW_PROP] = value;
    if (node.__mediaPromptPreview) {
        node.__mediaPromptPreview.value = value;
        node.__mediaPromptPreview.placeholder = value ? "" : TEXT.previewEmpty;
    }
    node.setDirtyCanvas?.(true, true);
}

function ensureOutputPreview(node) {
    if (!node || node.__mediaPromptPreview || typeof node.addDOMWidget !== "function") return;
    const wrap = document.createElement("div");
    wrap.className = "media-prompt-preview-wrap";
    const label = document.createElement("div");
    label.className = "media-prompt-preview-title";
    label.textContent = TEXT.previewTitle;
    const preview = document.createElement("textarea");
    preview.className = "media-prompt-preview";
    preview.readOnly = true;
    preview.spellcheck = false;
    preview.placeholder = TEXT.previewEmpty;
    wrap.append(label, preview);
    node.__mediaPromptPreview = preview;
    const widget = node.addDOMWidget("final_prompt_preview", "final_prompt_preview", wrap, {
        getValue: () => String(node.properties?.[OUTPUT_PREVIEW_PROP] || ""),
        setValue: (value) => setOutputPreview(node, value),
        serialize: false,
        getMinHeight: () => 110,
        margin: 10,
    });
    if (!widget) {
        node.__mediaPromptPreview = null;
        wrap.remove();
        return;
    }
    widget.serialize = false;
    setWidgetOption(widget, "serialize", false);
    setOutputPreview(node, node.properties?.[OUTPUT_PREVIEW_PROP] || "");
}
...
function installOutputNode(nodeType, nodeData) {
    ...
    nodeType.prototype.onConfigure = function onConfigureH3Output(info) {
        ...
        localizeNodeInstance(this);
        ensureOutputPreview(this);
        setOutputPreview(this, this.properties?.[OUTPUT_PREVIEW_PROP] || "");
        return result;
    };
    const originalExecuted = nodeType.prototype.onExecuted;
    nodeType.prototype.onExecuted = function onExecutedMiniMaxH3MediaPromptOutput(message) {
        const result = originalExecuted?.apply(this, arguments);
        const rawText = Array.isArray(message?.text) ? message.text[0] : message?.text;
        setOutputPreview(this, rawText == null ? "" : String(rawText));
        return result;
    };
}
...
function install() {
    ...
    style.textContent = `
            .media-prompt-preview-wrap {
                display: grid; grid-template-rows: auto minmax(72px, 1fr); width: 100%; height: 100%; min-height: 0;
                box-sizing: border-box; gap: 5px; padding: 6px; overflow: hidden;
            }
            .media-prompt-preview-title {
                color: var(--h3-native-widget-muted, rgba(255,255,255,.62)); font: 600 11px/1.2 Consolas, "Courier New", monospace;
                letter-spacing: 0; user-select: none;
            }
            .media-prompt-preview {
                width: 100%; height: 100%; min-height: 72px; box-sizing: border-box; resize: none; overflow: auto;
                padding: 7px 8px; border: 1px solid var(--h3-native-widget-outline, rgba(255,255,255,.14)); border-radius: 4px;
                outline: none; background: var(--h3-native-widget-bg, var(--comfy-input-bg, #222));
                color: var(--h3-native-widget-text, var(--input-text, #ddd)); font: 12px/1.45 Consolas, "Courier New", monospace;
                letter-spacing: 0; white-space: pre-wrap; overflow-wrap: anywhere; cursor: text;
            }
            .media-prompt-preview::placeholder { color: var(--h3-native-widget-muted, rgba(255,255,255,.38)); }
